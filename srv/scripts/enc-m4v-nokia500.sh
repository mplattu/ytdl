...
if [ -f "${SUBTITLES}" ]; then
    ...
    /usr/bin/mencoder -o "${FILE_OUT}" "${FILE_IN}" -oac mp3lame -ovc x264 -x264encopts subq=4:bframes=2:b_pyramid=normal:weight_b -vf scale=-3:480 -sub "${SUBTITLES}" -subcp utf8 >>${FILE_WAIT}
else
    # There are no subtitles file
    echo "Subtitles file not found: ${SUBTITLES}" >>${FILE_WAIT}
    /usr/bin/mencoder -o "${FILE_OUT}" "${FILE_IN}" -oac mp3lame -ovc x264 -x264encopts subq=4:bframes=2:b_pyramid=normal:weight_b -vf scale=-3:480 >>${FILE_WAIT}
fi
